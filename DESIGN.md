...
## 15. 自定义 Prompt 模板 (Custom Prompt System)

### 14.1 概述

用户可以在设置面板中创建自定义占位符，自动注册为 `{{name}}` Provider。创建后立即可在 Director Prompt、Script Wrapper 等任意模板中引用。

**存储**：`settings.customPrompts = [{ id, name, content, enabled }]`

**命名规则**：仅限 `\w+`（a-z, 0-9, _），与 `renderPrompt` 占位符正则一致。自动检测与内置 Provider 的命名冲突。

**两级控制**：
- **总开关** `customPromptsEnabled`：`false` 时全部停用
- **单独开关**：每个条目的 `enabled` 字段，独立控制

### 14.2 Provider 注册机制

```
initAll() → syncAll()
  ├ customPromptsEnabled=false → 全部 unregister
  └ customPromptsEnabled=true → 遍历 → enabled 的注册，关闭的注销

toggle(id) → syncOne(entry)
add/update/remove → syncOne(entry)
setMasterEnabled(false) → syncAll() 全部注销
```

Provider `render()` 直接返回 `entry.content`，不经过 `renderPrompt`（避免 Phase 1 嵌套递归）。

### 14.3 自引用检测

创建/编辑时检测内容是否包含 `{{name}}`——即该 Prompt 引用自身。检测到会 `toastr.warning` 提示，但不阻止保存。渲染时 `maxPasses` 保护，不会死循环。
...
```json
{
  ...
  "prompts": [{ "name": "myGuide", "content": "...", "enabled": true }]
}
```

**导入合并**：自动检测同名冲突，弹窗确认（覆盖 / 跳过）。

**配置档集成**：`assetManager` 抽屉中 `customPrompts` 随配置档 .zip 一起导出/导入。`applyProfile` 中的合并策略：增量追加（同名保留现有，不同名追加）。
...
| `customPromptsEnabled` | `true` | 总开关 |

文件：`systems/custom-prompts-system.js`、`ui/sections/customPrompts.js`
...
### 16.2 用户导入系统（User Provider/Capability Import）
...
## 17. Template Rendering Features

### 17.1 Locals（Per-call 占位符）
