...
const I18N = {
    zh: {
        ...
        configDrawerDirectorLlm: 'Director LLM',
        configDrawerWorldBooks: '世界书',
        ...
        summaryExportHint: '导出当前激活摘要为 JSON，或导入外部摘要（独立于生成系统，通过 <code>{{importedSummary}}</code> 注入 Prompt）。',
        ...
        identityHint: '自定义身份锚定提示词。在此书写指令告诉 LLM 如何看待角色档案、记忆、摘要等数据。<code>{{identity}}</code> 占位符不会自动注入，需手动放入 Director Prompt 或其他模板中。',
        identityPromptPlaceholder: '留空则使用内置默认模板（含 DSL 用法示范）',
        identityReset: '清空',
        ...
        profileExportHint: '将角色档案导出为独立 JSON 文件（含模板配置），或从外部 JSON 文件/预设导入档案。',
        ...
        profileImportTemplate: '同时导入模板配置（生成器 Prompt / Schema / 渲染模板）',
    ...
    en: {
        ...
        memoryCompressPromptHint: '用于将旧记忆摘要压缩为一条。占位符：<code>{{charName}}</code> <code>{{charDescription}}</code> <code>{{charPersonality}}</code> <code>{{memories}}</code>',
        memoryCompressPromptReset: '恢复默认',

        memoryExportTitle: '记忆导出 / 导入',
        memoryExportHint: '将角色记忆导出为 JSON 文件（含模板配置），或从外部文件导入。导入时支持精确匹配、名称匹配和手动指定目标角色。',
        ...
        memoryImportTemplate: '同时导入模板配置（Prompt / Schema / Render / Compress）',
        ...
        npcExportHint: '将 NPC 导出为独立 JSON 文件（含生成 Prompt），或从外部 JSON 文件/预设导入 NPC。',
    ...
    en: {
        ...
        configProfilesHint: 'Save, export, and import global config profiles. Create multiple profiles and switch between them, or export as .zip to share.',
        ...
        configDrawerDirectorLlm: 'Director LLM',
        configDrawerWorldBooks: 'World Books',
        ...
        summaryExportHint: 'Export the active summary as JSON, or import external summaries (independent of the generation system, injected via <code>{{importedSummary}}</code>).',
        ...
        identityHint: 'Custom identity anchoring prompt. Write instructions telling the LLM how to interpret character profiles, memories, summaries, etc. <code>{{identity}}</code> is opt-in — add it to your Director Prompt or any template manually.',
        identityPromptPlaceholder: 'Leave empty to use built-in default template (includes DSL usage examples)',
        identityReset: 'Clear',
        ...
        profileExportHint: 'Export character profiles as standalone JSON files (with template config), or import from external JSON files / presets.',
        ...
        profileImportTemplate: 'Also import template config (generator prompt / schema / render template)',
    ...
    en: {
        ...
        memoryCompressPromptHint: 'Prompt for summarizing old memories into one entry. Placeholders: <code>{{charName}}</code> <code>{{charDescription}}</code> <code>{{charPersonality}}</code> <code>{{memories}}</code>',
        memoryCompressPromptReset: 'Reset',

        memoryExportTitle: 'Memory Export / Import',
        memoryExportHint: 'Export character memories as JSON (with template config), or import from external files. Supports avatar exact match, name match, and manual target selection.',
        ...
        memoryImportTemplate: 'Also import template config (Prompt / Schema / Render / Compress)',
        ...
        npcExportHint: 'Export NPCs as standalone JSON files (with generation prompt), or import from external JSON files / presets.',
